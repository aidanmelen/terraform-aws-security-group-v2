...
func TestTerraformManagedRulesExample(t *testing.T) {
	...
	expectedIngress4 := fmt.Sprintf("map[cidr_blocks:<nil> description:managed by Terraform from_port:5432 id:sgrule-1111111111 ipv6_cidr_blocks:[2001:db8::/64] prefix_list_ids:<nil> protocol:tcp security_group_id:%s self:false source_security_group_id:<nil> timeouts:<nil> to_port:5432 type:ingress]", actualSecurityGroupId)
