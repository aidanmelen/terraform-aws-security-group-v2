...
func TestTerraformMatrixRulesExample(t *testing.T) {
	...
	// assign expected
	expectedIngress0 := fmt.Sprintf("map[cidr_blocks:[10.0.0.0/24 10.0.1.0/24] description:matrix managed rule from_port:443 id:sgrule-1111111111 ipv6_cidr_blocks:[] prefix_list_ids:[%s] protocol:tcp security_group_id:%s self:false source_security_group_id:<nil> timeouts:<nil> to_port:443 type:ingress]", actualDataAwsPrefixListPrivateS3Id, actualSecurityGroupId)
	expectedIngress1 := fmt.Sprintf("map[cidr_blocks:<nil> description:matrix managed rule from_port:443 id:sgrule-1111111111 ipv6_cidr_blocks:<nil> prefix_list_ids:<nil> protocol:tcp security_group_id:%s self:false source_security_group_id:%s timeouts:<nil> to_port:443 type:ingress]", actualSecurityGroupId, actualDataAwsSecurityGroupDefaultId)
	expectedIngress2 := fmt.Sprintf("map[cidr_blocks:<nil> description:matrix managed rule from_port:443 id:sgrule-1111111111 ipv6_cidr_blocks:<nil> prefix_list_ids:<nil> protocol:tcp security_group_id:%s self:true source_security_group_id:<nil> timeouts:<nil> to_port:443 type:ingress]", actualSecurityGroupId)
	...
	expectedEgress0 := fmt.Sprintf("map[cidr_blocks:[10.0.0.0/24 10.0.1.0/24] description:HTTPS from_port:443 id:sgrule-1111111111 ipv6_cidr_blocks:<nil> prefix_list_ids:<nil> protocol:tcp security_group_id:%s self:false source_security_group_id:<nil> timeouts:<nil> to_port:443 type:egress]", actualSecurityGroupId)
	expectedEgress1 := fmt.Sprintf("map[cidr_blocks:<nil> description:HTTPS from_port:443 id:sgrule-1111111111 ipv6_cidr_blocks:<nil> prefix_list_ids:<nil> protocol:tcp security_group_id:%s self:false source_security_group_id:%s timeouts:<nil> to_port:443 type:egress]", actualSecurityGroupId, actualDataAwsSecurityGroupDefaultId)
