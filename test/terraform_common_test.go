...
func TestTerraformCommonRulesExample(t *testing.T) {
	...
	expectedPublicHttpIngress1 := fmt.Sprintf("map[cidr_blocks:[0.0.0.0/0] description:HTTP from public from_port:80 id:sgrule-1111111111 ipv6_cidr_blocks:[::/0] prefix_list_ids:<nil> protocol:tcp security_group_id:%s self:false source_security_group_id:<nil> timeouts:<nil> to_port:80 type:ingress]", actualPublicHttpSecurityGroupId)
	expectedPublicHttpEgress0 := fmt.Sprintf("map[cidr_blocks:[0.0.0.0/0] description:All to public from_port:0 id:sgrule-1111111111 ipv6_cidr_blocks:[::/0] prefix_list_ids:<nil> protocol:-1 security_group_id:%s self:false source_security_group_id:<nil> timeouts:<nil> to_port:0 type:egress]", actualPublicHttpSecurityGroupId)
